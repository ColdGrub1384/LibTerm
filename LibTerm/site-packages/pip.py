...
if False:
    ...
    PYTHONISTA_BUNDLED_MODULES = []
    if sys.version[0] == '2':
        SITE_PACKAGES_FOLDER = os.path.expanduser("~/Documents/site-packages2")
    else:
        SITE_PACKAGES_FOLDER = os.path.expanduser("~/Documents/site-packages3")
    ...
    def remove(self, pkg_name):
        if self.config.module_exists(pkg_name):
            ...
            if files_installed:
                ...
                if os.path.isdir(SITE_PACKAGES_DIR_NAME+'/'+pkg_name.lower()):
                    shutil.rmtree(SITE_PACKAGES_DIR_NAME+'/'+pkg_name.lower())
                elif os.path.isfile(SITE_PACKAGES_DIR_NAME+'/'+pkg_name.lower()+'.py'):
                    os.remove(SITE_PACKAGES_DIR_NAME+'/'+pkg_name.lower()+'.py')
                else:
                    print('Package may have been removed externally without using pip. Deleting from registry ...')
...
class PyPIRepository(PackageRepository):
    ...
    def download(self, pkg_name, ver_spec, wheel_priority=False):
        ...
        worker = wget([source['url'], '-o', '{}/{}'.format(os.getenv('TMPDIR'), source['filename'])])
        sleep(1)
...
class GitHubRepository(PackageRepository):
    ...
    def download(self, owner_repo, ver_spec):
        ...
        wget(['https://github.com/{0}/{1}/archive/{2}.zip'.format(owner, repo, release), '-o', '{}/{}.zip'.format(os.getenv('TMPDIR'), repo)])
        sleep(1)
...
class UrlRepository(PackageRepository):
    ...
    def download(self, url, ver_spec):
        ...
        wget([url, '-o', '{}/{}'.format(os.getenv('TMPDIR'), archive_filename)])
        sleep(1)
